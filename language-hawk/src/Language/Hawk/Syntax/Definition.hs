...
import Language.Hawk.Syntax.Datatype
...
instance Semigroup (Lib t v) where
  (<>) l1 l2 = Lib
...
instance (Pretty var, Pretty (term var)) => Pretty (Lib term var) where
  pretty = undefined

instance (Pretty term) => Pretty (Def term) where
  pretty = undefined

...
liftPatEq f g pat1 pat2 = case (pat1, pat2) of
    ...
    (PAnnot t1 p1, PAnnot t2 p2) -> liftPatEq f g p1 p2 && f t1 t2
...
instance (Eq typ, Eq b) => Eq (Pat typ b) where
  (==) p1 p2 = case (p1, p2) of
    ...
    (PAnnot t1 p1, PAnnot t2 p2) -> t1 == t2 && p1 == p2
...
instance Bitraversable Pat where
  bitraverse f g pat = case pat of
    ...
    PAnnot t p -> PAnnot <$> f t <*> bitraverse f g p
